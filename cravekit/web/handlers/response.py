...
def Authenticate(x_api_key: Union[str, None] = Header(None)):
    if x_api_key in config.auth.allowed_tokens or x_api_key == config.auth.admin_token or config.auth.auth is False:
        return True
    else:
        return False
...
def handle_response(response, original_image) -> Response:
    ...
    if isinstance(response, dict):
        if response["type"] == "jpg":
            ...
            response_object = Response(content=response["data"][0].read(), media_type='zip')
